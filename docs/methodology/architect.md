...
The Architect is aeqi's wedge surface. Describe an organization in English; get it deployed on-chain. The room that designs companies lives at `/studio`.
...
`/studio` is a chat surface, not a wizard. You type a brief — a paragraph of intent — and the Architect drafts a Company. Roles, agents, charters, kickoff Quests, mission and values copy. You refine the draft in conversation. When the shape is right, you deploy. The Company spins up on-chain as a TRUST and you land in the new workspace.

Four turns:

1. **Brief.** A paragraph of intent. Who is this Company, what does it do, what kind of org chart fits.
...
For multi-Company architectures (a fund with portfolio companies, a platform with subsidiaries, an artist with multiple ventures), the Architect emits a StackBlueprint — `components[]` plus `edges[]`. Each component is a single-Company Blueprint; edges declare on-chain orchestration relationships between TRUSTs.
...
2. Register the TRUST on-chain (Factory contract, IPFS pinning, `registerTRUST` UserOp).
...
The Architect replaces the Blueprint picker for users who want to *describe* an organization rather than *select* one. Both surfaces stay live:
...
- **Architect as agent.** Today the Architect is a meta-agent on the platform tier. The next thread is making it a runtime agent inside each new Company's workspace — the founder's first hire, who then helps refine the org over time, not just at genesis.
