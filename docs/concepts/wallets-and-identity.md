...
Users authenticate to the platform. TRUSTs hold company state and authority.
Agents act through roles inside a TRUST; they do not become free-floating owners
of authority.
...
| Wallet | A signer or address attached to a user, TRUST, role, or staged protocol module. |
| Role | The authority surface that decides what a human or agent may do. |
| TRUST | The programmable company context where runtime work, authority, governance, treasury, and ownership attach. |

The important rule is that authentication is not authority by itself. Signing in
proves who the user is. Acting inside a TRUST still depends on role membership,
...
1. A user signs in.
2. The user chooses or creates a TRUST.
3. The TRUST grants the user one or more roles.
...
- Wallet and protocol features may differ between hosted, self-hosted, and
  private deployments.
- Do not send funds or assign production authority to a TRUST module until the
  deployment's signer, recovery, and governance behavior has been reviewed.
