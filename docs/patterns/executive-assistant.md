...
A shared EA serving the C-suite collectively, not any one exec. The reference shape: `<company> EA` (e.g., `aeiq EA`) — neutral voice, no decisional authority, listens always, responds when called.
...
| Field | Value |
|---|---|
| Display name | `<company> EA` (e.g., `aeiq EA`) |
...
## Reference: AEIQ dogfood

Live in the AEIQ tenant:

- Agent `c0ba8326`
- Role `98488ed4`
- Channel `d60c36bb-93fc-4555-aefc-f0eed03e713f` (Telegram bot `@aeiq_ea_bot`, allowed group `-5133857242` "ÆIQ").
